...
class TaxConfig(Base):
    ...
    tax_name: Mapped[uuid.UUID] = mapped_column(String(100), nullable=False)
...
class PpmPayment(Base):
    ...
    status: Mapped[uuid.UUID] = mapped_column(String(20), nullable=False, default="pending")  # pending/paid/overdue
    folio: Mapped[uuid.UUID | None] = mapped_column(String(50), nullable=True)
...
class TaxResult(Base):
    ...
    period_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("accounting.periods.id"), nullable=False, unique=True, index=True
    )
