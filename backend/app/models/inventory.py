...
class Product(Base):
    ...
    sku: Mapped[uuid.UUID] = mapped_column(String(100), unique=True, nullable=False, index=True)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    description: Mapped[uuid.UUID | None] = mapped_column(Text, nullable=True)
...
class StockMovement(Base):
    ...
    product_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("inventory.products.id"), nullable=False, index=True
    )
    movement_type: Mapped[uuid.UUID] = mapped_column(String(20), nullable=False)  # in/out/adjustment
    ...
    reference: Mapped[uuid.UUID | None] = mapped_column(String(100), nullable=True)
